...
#: Archived entries keep their specification, generator and tests, but are not
#: part of the chemistry catalog: listings, the documentation catalog and the
#: LLM tools leave them out, while `generate_network(id)` still runs them.
#: Restoring one means deleting its `archived:` line.
ARCHIVES = {
...
def active(entries: Iterable[Chemistry]) -> list[Chemistry]:
    """The chemistry catalog proper: every entry that is not archived."""
    return [c for c in entries if c.archived is None]
...
def render_index(entries: list[Chemistry]) -> str:
    ...
    done = [c for c in main if c.implemented]
    fidelity = Counter(c.fidelity for c in done)
    kin = sum(1 for c in main if "kinetics" in c.tiers)
    thermo = sum(1 for c in main if "thermodynamics" in c.tiers)
    constructive = sum(1 for c in main if c.constructive)
    out.append(
    ...
    if archived:
        out.append("\n## Archive\n")
        out.append(
            "Not part of the chemistry catalog: left out of listings and of the "
            "LLM tools, but kept with their specification, generator and tests, "
            "and still runnable by id.\n"
        )
        for group in sorted({c.archived for c in archived}):
            members = [c for c in archived if c.archived == group]
            out.append(f"\n### {group} ({len(members)}): {ARCHIVES.get(group, '')}\n")
            out += _index_table(members)
    return "\n".join(out) + "\n"
...
def main(argv: list[str]) -> int:
    ...
    if cmd == "validate":
        ...
        archived = len(entries) - len(active(entries))
        print(f"{len(entries) - archived} chemistries"
    ...
    if cmd == "status":
        print(render_status(active(entries)))
        return 0
    ...
    if cmd == "query":
        ...
        for c in (entries if "--all" in args else active(entries)):

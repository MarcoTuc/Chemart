"""Random Boolean networks (book 18.4.2) and RBN World (book 18.4.2, 10.7.3). Catalog id: rbn.

model = "rbn": Kauffman's classic RBN. N nodes, node i reads k_i distinct
inputs (k_i = K, or drawn with mean K) through a random truth table whose
outputs are 1 with probability function_bias. The network is the Boolean
network written as reactions: node i has the species x{i}_0 and x{i}_1, and
truth-table row r of node i is the reaction in which the input species of row
r catalyse the switch of x{i} to the row's output. With one molecule per node
this is exactly the node-by-node (asynchronous) semantics; the synchronous map
and its attractors are in extras.analysis.

model = "rbn-world": Faulconbridge, Stepney, Miller & Caves's RBN World. An
atom is a bonding RBN (bRBN, b = 2 bonding sites); a molecule is a tree of
bRBNs whose bonds are chosen, kept and broken by an emergent property of the
bRBNs' attractors. The network is the set of reactions observed in a
well-stirred collision run (status "observed").
"""
...
def generate(p, rng):
    ...
    if p.K_distribution != "constant":
        raise ValueError("rbn-world atoms have a fixed K (Faulconbridge et al.); use K_distribution='constant'")
    if p.N * p.K < SITES:
...
def _classic(p, rng):
    ...
    listed = [{"length": a["length"], "basin_size": a["basin_size"],
               "cycle": [_bits(s, N) for s in a["cycle"][:64]]} for a in found]
...
# ============================================================================
# RBN World: species identity and the collision run
# ============================================================================
class Registry:
    ...
    def _canon(self, node, elem, bonds):
        ...
        if not node.children:
            ...
            return f"{letter}:{_bits(node.state[a], N)}", letter, [a]
        ...
        for order in orderings:
            ...
            key = (";".join(f"{u[0]}.{u[1]}.{u[2]}={v[0]}.{v[1]}.{v[2]}" for u, v in links),
                   "".join(_bits(node.state[a], N) for a in atoms), atoms)
...
def _world(p, rng):
    ...
    fired, final = soup(react, population, p.collisions, rng, arity=2)
    ...
    conservation = [{"name": f"atoms of element {letters[e]}",
                    ...
                    for e in range(p.elements)]
    sizes = {ids[c]: len(registry.molecules[c][0]) for c in used}
    return Network(
        species=species,
        reactions=reactions,
        status="observed",
        ...
        extras={
            ...
            "elements": [{
                ...
                "initial_state": _bits(el.initial, N),
            ...
            "analysis": {
                ...
                "largest_molecule_atoms": max(sizes.values()),

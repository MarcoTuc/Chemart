<!-- Generated by tools/gen_catalog_pages.py from catalog/chemistries/social-communication-ac.yaml. Edit those, not this file. -->
...
**S — molecules** (*explicit* — listed up front). the N possible activities (messages) 1..N, one species a_i per activity; a molecule is the message currently displayed on an agent's sign, so the population of displayed signs is a multiset of n_agents molecules. The species are communications, not persons: agents are the stateful context in which a reaction happens, and their memories are in extras.agents.

Messages carry no internal structure ("There is no a priori relationship between messages", Sec. 2.1), so Species.structure is None.

**R — reactions** (*implicit* — computed by an algorithm from the molecules that meet; molecules per reaction: 2). a_i + a_k -&gt; a_i + a_j : Ego observes Alter's displayed message i (a catalyst, Alter does not act), replaces its own displayed message k, and selects activity j with probability w_AP[j] computed from its two memory matrices; Ego, Alter and n_observers agents then memorise the event (i, j). The observed events of a run are returned with firing counts.

w_AV = normalize((1 - alpha) w_EE + alpha w_EC + c_f/N) (eq. 2) and w_AP = normalize(w_AV^gamma) (eq. 3), with w_EE[i] = lookup(M_ego or M_alter, received, i), w_EC[i] = f_certainty(lookup(M_alter, i)) and f_certainty(p) = 1 + sum_i p_i log_N p_i (eq. 1). The outcome depends on agent state, so this is not a state-free reaction rule; the rule itself is recorded in extras.interaction_law.

**A — reactor**: `well-stirred-multiset` (a well-stirred pot of discrete molecules that meet at random, with no notion of position).

*How the population is bounded:* none: every agent always displays exactly one sign, so the population is constant at n_agents

One step is one activity selection. interaction='dyadic' is the paper's dyadicWorld (two agents acting alternately, Sec. 2), 'population' its multiWorld (Ego and Alter drawn at random each step, Sec. 6.1/6.13).
...
| `alpha` | `float` | `0.0` | selection | fraction of expectation-certainty in the activity value (eq. 2): 0 = expectation-expectation only, 1 = expectation-certainty only <br>`0.0` … `1.0` · *range:* paper: 0.0, 0.5, 1.0 |
...
Banzhaf, W. & Yamamoto, L. (2015). *Artificial Chemistries*. MIT Press. Section 20.4; the book's bibliography entries [234], [235].
...
*Specification: `catalog/chemistries/social-communication-ac.yaml` · explainer: `catalog/explainers/social-communication-ac.md` · generator: `chemart/chemistries/social_communication_ac.py` · tests: `tests/chemistries/test_social_communication_ac.py`*

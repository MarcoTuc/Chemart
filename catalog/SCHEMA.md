...
| `duration` | the parameter that says how long the evolve face runs, in `clock` units; required when the module defines `evolve`. Its `min` must be 0, the value that runs the process until whoever reads its frames stops |

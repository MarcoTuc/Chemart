...
def test_a_process_can_run_without_end():
    import itertools

    frames = list(itertools.islice(chemart.evolve_frames("alchemy", seed=1, collisions=0), 3))
    assert len(frames) == 3 and frames[0].t == 0 and frames[-1].t > 0
    with pytest.raises(ValueError, match="evolve_frames"):
        chemart.evolve("alchemy", seed=1, collisions=0)      # the whole run never arrives
    with pytest.raises(ValueError, match="no network is ever finished"):
        chemart.generate_network("srsim", seed=1, steps=0)   # a gas generated by running it
...
def test_cli_evolve_and_measure(capsys, tmp_path):
    ...
    assert cli_main(["evolve", "brusselator", "--endless"]) == 2
    assert "no length parameter" in capsys.readouterr().err
    assert cli_main(["evolve", "alchemy", "-p", "collisions=0", "--format", "json"]) == 2
    assert "no whole trajectory" in capsys.readouterr().err
